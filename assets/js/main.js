...
document.querySelectorAll('.tech h2 + i, .tech h2 ~ i').forEach(function (icon) {
    icon.addEventListener('mouseover', function () {
        toggleDeviconWordmark(icon, true);
    });
    icon.addEventListener('mouseout', function () {
        toggleDeviconWordmark(icon, false);
    });
});

function toggleDeviconWordmark(icon, addWordmark) {
    icon.classList.forEach(function (cls) {
        if (cls.startsWith('devicon-') && !cls.endsWith('-wordmark')) {
            if (addWordmark && !noWordmarkClasses.includes(cls)) {
                icon.classList.replace(cls, cls + '-wordmark');
            }
        } else if (cls.startsWith('devicon-') && cls.endsWith('-wordmark')) {
            if (!addWordmark && !noWordmarkClasses.includes(cls)) {
                icon.classList.replace(cls, cls.replace('-wordmark', ''));
            }
        }
    });
}

const noWordmarkClasses = [
